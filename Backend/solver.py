...
if __name__ == "__main__":
    # Accept cube state from command line
    cube_state = None
    try:
        cube_state = sys.argv[1] if len(sys.argv) > 1 else "DLURRDFFUBBLDDRBRBLDLRBFRUULFBDDUFBRBBRFUDFLUDLUULFLFR"
    except Exception:
        cube_state = "DLURRDFFUBBLDDRBRBLDLRBFRUULFBDDUFBRBBRFUDFLUDLUULFLFR"

    # Construct cube + solve, but trap errors and print them to stderr only
    try:
        c = cube.Cube(cube_state)
        solver = Solver(c)
        solver.solve()
    except Exception as e:
        # send error to stderr (Node will get this in stderr)
        sys.stderr.write(str(e) + "\n")
        # non-zero exit to indicate failure
        sys.exit(1)

    # success: print only the moves (single line)
